...
pub trait JetHL: Jet {
    fn source_type(jet: Self) -> Vec<AliasedType>;
    fn target_type(jet: Self) -> AliasedType;

    fn assert_jet() -> Self;

    fn filter(jet: Self, _name: &JetName) -> Result<Self, Error> {
        Ok(jet)
    }
}
...
impl JetHL for Elements {
    fn source_type(jet: Self) -> Vec<AliasedType> {
        match jet {
            ...
            Elements::Maj1 | Elements::XorXor1 | Elements::Ch1 => {
                vec![U1.into(), U1.into(), U1.into()]
            }
            Elements::Maj8 | Elements::XorXor8 | Elements::Ch8 => {
    ...
    fn filter(jet: Self, name: &JetName) -> Result<Self, Error> {
        if jet == Elements::CheckSigVerify || jet == Elements::Verify {
            return Err(Error::JetDoesNotExist(name.clone()));
        }
...
mod tests {
    ...
    fn compatible_target_type() {
        for jet in Elements::ALL {
            let resolved_ty = Elements::target_type(jet).resolve_builtin().unwrap();
            let structural_ty = StructuralType::from(&resolved_ty);
