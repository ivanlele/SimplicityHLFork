...
use crate::jet::{
    external::{
        backend::ExternalJetLib, loaders::dynlib::Library, ExternalJet, EXTERNAL_JET_ABI_VERSION,
    },
...
impl ExternalJetDynamicLib {
    ...
    pub unsafe fn load(library: Library) -> Result<Self, Box<dyn std::error::Error>> {
        let external_jet_abi_version: unsafe extern "C" fn() -> u32 =
            library.symbol("external_jet_abi_version")?;
        let version = external_jet_abi_version();
        if version != EXTERNAL_JET_ABI_VERSION {
            return Err(format!(
                "External jet ABI version mismatch: expected {}, found {version}",
                EXTERNAL_JET_ABI_VERSION
            )
            .into());
        }
