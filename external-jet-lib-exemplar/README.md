...
```rust
simplicityhl::jet::external::init_external_jet_lib("/path/to/libexternal_jet_lib_exemplar.dylib")
...
cargo build -p external-jet-lib-exemplar

# Run the consumer binary, pointing it at the compiled dylib
cargo run -p external-jet-lib-exemplar --bin external-lib-consumer -- \
    target/debug/libexternal_jet_lib_exemplar.dylib
```
