...
The embedder must provide the imports under `simplicityhl-plugin` with the exact
C ABI declared by the compiler. The compiler checks the plugin's external jet
ABI version before making its first jet call.
